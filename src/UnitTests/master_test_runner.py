...
class UltimateTestSuite:
    ...
    def test_complete_workflow(self):
        ...
        # Realistic program: Fibonacci sequence
        fibonacci_program = """
        ...
        main:
            ...
            addi x4, x4, 2      # index = 2 (start from fib(2))
            
        fib_loop:
            beq x4, x3, done    # if index == 5, done
            
            # Calculate next fibonacci
            add x5, x1, x2      # fib(n) = fib(n-1) + fib(n-2)
            
            # Store result
            sw x5, 0(x4)        # mem[index] = fib(n)
            
            # Update for next iteration
            add x1, x0, x2      # fib(n-2) = fib(n-1)
            add x2, x0, x5      # fib(n-1) = fib(n)
            addi x4, x4, 1      # index++
            
            bne x4, x3, fib_loop # continue if not done
            
        done:
            # Load some results for verification
            lw x6, 0(x0)        # fib(0) = 0
            lw x7, 1(x0)        # fib(1) = 1
            lw x8, 2(x0)        # fib(2) = 1
            lw x9, 3(x0)        # fib(3) = 2
        ...
        # Complete workflow: Assembly → Load → Execute → Verify
        assembler = RiscVAssembler()
        processor = RiscVProcessor(64, 64)
        ...
        try:
            machine_code = assembler.assemble_file(temp_asm)
            if not machine_code:
            ...
            # Step 2: Load from binary
            processor.instruction_memory.load_from_binary_file(temp_bin)
            ...
            expected_fib = [0, 1, 1, 2, 3]  # First 5 fibonacci numbers
            actual_results = []
            
            for i in range(5):
                value = processor.data_memory.read_word(0x1000 + i)
                actual_results.append(value)
            
            for i, (expected, actual) in enumerate(zip(expected_fib, actual_results)):
                if expected != actual:
                    raise AssertionError(f"Fibonacci({i}): expected {expected}, got {actual}")
            
            # Verify registers contain last loaded values
            register_results = [processor.register_file.read(i) for i in range(6, 11)]
            
            print(f"   ✓ Assembly successful: {len(machine_code)} instructions")
            print(f"   ✓ Binary file save/load successful")
            print(f"   ✓ Execution completed in {processor.cycle_count} cycles")
            print(f"   ✓ Fibonacci sequence calculated correctly: {actual_results}")
            print(f"   ✓ Register verification successful: {register_results}")
            
            return {
                'fibonacci_sequence': actual_results,
                'execution_cycles': processor.cycle_count,
                'instruction_count': len(machine_code),
                'register_values': register_results
            }
            
        finally:
            os.unlink(temp_asm)
            if 'temp_bin' in locals():
                os.unlink(temp_bin)
